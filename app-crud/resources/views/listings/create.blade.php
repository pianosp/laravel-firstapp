<x-layout>
    <x-card class="max-w-lg mx-auto mt-24">
        ...
        <form method="POST" action="/listing" {{--action ไปที่ routeนั้น ก็ไปไล่ดูว่า route นั้นทำไรต่อ--}} enctype="multipart/form-data">
            @csrf
            <div class="mb-6">
            ...
            <div class="mb-6">
                ...
                <input type="text" class="border border-gray-200 rounded p-2 w-full" name="tag"
                    placeholder="Example: Laravel, Backend, Postgres, etc" value="{{old('tag')}}" />
                    @error('tag')
                    <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                @enderror
            </div>
